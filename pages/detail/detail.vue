<template>
	<view class="contaioner">
		...
		<uni-group class="seat-group" title="座位表" mode="card">
			...
			<button class="refresh-btn" @click="onrefreshBtnClick" type="default" size="mini" :loading="loading">刷新</button>
		</uni-group>
		...
		<uni-group class="detail-group" title="座位信息" mode="card" v-if="selectedItem">
			<view class="detail">
				...
				<view v-if="selectedItem?.selectTime">
					选择时间：
					<uni-dateformat :date="new Date(selectedItem?.selectTime) - 30000" format="M月d日 h时m分" :threshold="[0, 3600000]"></uni-dateformat>
				</view>
			</view>
		</uni-group>
		...
		<button
			...
			:disabled="(selectedItem?.status == 3 && selectedItem?.stuInfo?.openid != userinfo._openid) || !selectedItem"
			:type="selectedItem?.stuInfo?.openid == userinfo._openid ? 'warn' : 'primary'"
			:loading="loading"
		>
			{{ selectedItem?.stuInfo?.openid == userinfo._openid ? '撤销选座' : selectedItem?.status == 3 ? '已被选择' : '选择座位' }}
		</button>
		...
		<view @click="() => (showDrawer = false)" class="drawer" :class="{ hide: !showDrawer }">
			<view @click.stop.prevent class="drawer-content">
				...
				<button @click="onSubmitBtnClick" :type="isSelectSubmit ? 'primary' : 'warn'" :loading="loading">确认</button>
			</view>
		</view>
	</view>
</template>
...
const onSelectBtnClick = async () => {
	...
	if (stuInfo.class == '' || stuInfo.id == '' || stuInfo.name == '') {
		...
		if (!confirm) return;
		return uni.navigateTo({ url: '/pages/myseat/myseat' });
	}
